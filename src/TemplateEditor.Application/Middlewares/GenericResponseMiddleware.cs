        ...
        try
        {
            ...
            if (statusCode >= 200 && statusCode < 300)
            {
                var data = string.IsNullOrWhiteSpace(responseText) 
                           ? null 
                           : JsonSerializer.Deserialize<object>(responseText, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                           
                genericResponse = new
                {
                    Success = true,
                    Data = data,
                    Message = ExceptionCodes.Success,
            ...
            else
            {
                var errorDetails = new { 
                    ...
                    Message = ExceptionCodes.UnexpectedException,
                    Details = (object?)null
                };
                     ...
                     try
                     {
                         ...
                         if (abpError.TryGetProperty(GenericConstants.Error, out var errorElement))
                         {
                            errorDetails = new {
                                ...
                                Message = errorElement.GetProperty(GenericConstants.Message).GetString() ?? ExceptionCodes.UnexpectedException,
